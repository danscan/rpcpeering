...
import { upgradeWCUri, parseUpgradeWCUri, base64UrlEncode, uriIncludesSessionProposal } from './WalletConnectInterop';
...
const WC_URI_WITH_PROPOSAL = `${WC_URI}&org.rpcpeering.proposal=${encodeURIComponent(base64UrlEncode(JSON.stringify(PROPOSAL)))}`;

describe('uriIncludesSessionProposal', () => {
  it('should return true if the WC URI includes a peering proposal', () => {
    expect(uriIncludesSessionProposal(WC_URI_WITH_PROPOSAL)).toBe(true);
  });

  it('should return false if the WC URI does not include a peering proposal', () => {
    expect(uriIncludesSessionProposal(WC_URI)).toBe(false);
  });
});

describe('upgradeWCUri', () => {
  it('should upgrade a WC URI to a peering URI', () => {
    const upgraded = upgradeWCUri(WC_URI, PROPOSAL);
...
describe('parseUpgradeWCUri', () => {
  it('should parse a peering URI from a WC URI', () => {
    const parsed = parseUpgradeWCUri(WC_URI_WITH_PROPOSAL);
